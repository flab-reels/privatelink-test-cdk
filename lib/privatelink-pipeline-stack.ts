...
export class PrivatelinkPipelineStack extends cdk.Stack {
  public readonly tagParameterContainerImage: ecs.TagParameterContainerImage;
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    super(scope, id, props);
      const appEcrRepo = new ecr.Repository(this, 'privatelink-ecr-repository',{
        repositoryName:'privatelink-repository',

      });
      const appCodeDockerBuild = new codebuild.PipelineProject(this, 'privatelink-docker-build', {
        projectName: "privatelink-codebuild",
        environment: {
      ...
      const cdkCodeBuild = new codebuild.PipelineProject(this, 'CdkCodeBuildProject', {
        projectName: "privatelink-cdk-codebuild",
        environment: {
      ...
      const appCodeBuildAction = new codepipeline_actions.CodeBuildAction({
        actionName: 'privatelink-docker-build-action',
      ...
      new codepipeline.Pipeline(this, 'privatelink-code-pipeline', {
        // artifactBucket: new s3.Bucket(this, 'ArtifactBucket', {
        //     bucketName:'user-cdk-bucket',
        //     removalPolicy: cdk.RemovalPolicy.DESTROY,
        // }),
        pipelineName:"privatelink-pipeline",
          ...
          },
          {
            ...
            actions: [
              new codepipeline_actions.CloudFormationCreateUpdateStackAction({
                actionName: 'Privatelink_CloudFormation_CodeDeploy',
                stackName: 'PrivatelinkEcsStackDeployedInPipeline',
                // this name has to be the same name as used below in the CDK code for the application Stack
                templatePath: cdkCodeBuildOutput.atPath('PrivatelinkEcsStackDeployedInPipeline.template.json'),
  ...
  public createUserGithubSourceAction(sourceOutput: codepipeline.Artifact): codepipeline_actions.GitHubSourceAction {
      return new codepipeline_actions.GitHubSourceAction({
        actionName: 'Privatelink-pipeline-github',
        owner: 'flab-reels',
        repo: 'privatelink-test',
  ...
  public createCDKGithubSourceAction(sourceOutput: codepipeline.Artifact): codepipeline_actions.GitHubSourceAction {
      return new codepipeline_actions.GitHubSourceAction({
        actionName: 'Privatelink-pipeline-cdk',
        owner: 'flab-reels',
        repo: 'privatelink-test-cdk',
        oauthToken: SecretValue.secretsManager('github_source_accesskey'),
        output: sourceOutput,
        branch: 'master', // default: 'master'
      });
...
export class PrivatelinkEcsAppStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: EcsAppStackProps) {
    ...
    const cluster = new ecs.Cluster(this, 'Cluster', {
      vpc,
      clusterName:"privatelink-cluster"
    })
    const fargateTaskDefinition = new ecs.FargateTaskDefinition(this, 'privatelinkFargateDefinition', {
      memoryLimitMiB: 1024,
      cpu: 512,

    });
    const container = fargateTaskDefinition.addContainer("privatelinkServiceContainer", {
    ...
    const secGroup = new SecurityGroup(this, 'privatelink-sg', {
      allowAllOutbound:true,
      securityGroupName: "privatelink-sg",
      vpc:vpc,
    });
    ...
    const listener = loadBalancer.addListener('privatelink-listener',{
      port:80,
    })
    listener.addTargets('privatelink-target',{
      port:80,
      targets:[service],
      healthCheck:{
    ...
    new cdk.CfnOutput(this, "privatelink-VPC-id",{value: vpc.vpcId})
    new cdk.CfnOutput(this,"privatelink-nlb-arn",{value: loadBalancer.loadBalancerArn})
    new cdk.CfnOutput(this,"privatelink-nlb-dns",{value: loadBalancer.loadBalancerDnsName})
  }
}
